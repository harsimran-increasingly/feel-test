    ...
    switch (INC.config.pageType) {
        ...
        case "pdp":
            ...
            setTimeout(function () {
                ...
                // Out Of Stock
                var oos_element = document.querySelector('.oos-subsku-name')
                if (oos_element) {
                    if (oos_element.innerText !== "") {
...
INC.classes.ProductObj = function (product) {
    ...
    if (product["SpecialPrice"] == 0 || product["SpecialPrice"] == 0.00) {
                ...
                switch (INC.config.pageType) {
                    case "pdp":
                        ...
                        try {
                            ...
                            // var loadBundles= incPriceMismatch('1.00','1.00')
                            var loadBundles = incPriceMismatch(priceCheckPrice, priceCheckSpecialPrice)
                            if (loadBundles == false) return
    ...
    if (document.querySelector('.inc_pdp_block') != null && document.querySelector('.inc_sidebar_modal_block.active') == null && onloadpdp == true) {
        if (navigator.userAgent.toLowerCase().indexOf('safari') >= 0 && navigator.userAgent.toLowerCase().indexOf('chrome') == -1 && window.innerWidth < 1337) {
            jQuery("html, body").animate({
                scrollTop: jQuery(".inc_pdp_block").offset().top - 150
            }, 0);
        }
    }
    setTimeout(function () {
    ...
    elAddBlock.addEventListener("click", function () {
        ...
        if (productModuleBlock.querySelector('.img_color_swatch') != null) {
            if (productModuleBlock.querySelector(getSelectorClassName(classNames, "productModalBlock")).querySelector('.inc_product_desc_att_color_list_item_block.active') != null) {
                ...
                $('.inc_product_modal_block.active .inc_product_block').animate({
                    scrollTop: $('.inc_product_desc_att_block').height() + 600
                }, 'slow');
                productModuleBlock.querySelector('.inc_product_modal_block .inc_product_desc_add_err_text_block').innerText = "Please Select option"
            }
        } else if (productModuleBlock.querySelector(getSelectorClassName(classNames, "productModalBlock")).querySelector('.inc_product_desc_att_size_list_item_img_block.avail') != null) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (window.location.host == "www.feelunique.com" || window.location.host == "staging.feelunique.com") {
                window.FeelUnique.Plugins.responsiveHeader.prototype.updateBasketQuantity({
                    quantity: parseInt(data.analytics.qty)
                });
            } else {
                if (document.querySelectorAll('button[aria-label="Cart"] span div')[2] != undefined) {
                    var old_qty = document.querySelectorAll('button[aria-label="Cart"] span div')[2].innerText
        ...
        error: function (jqXHR, textStatus, errorMessage) {
            ...
            addToCartError(productObj['id'], errorMessage)
            if (INC.config.pageType == "cartPage") {
    ...
    function callprodA() {
        $.ajax({
            ...
            error: function (jqXHR, textStatus, errorMessage) {
                ...
                addToCartError(pid, errorMessage)
                if (url2 == "") {
                    getTrackingPixel()
...
function clickListener(e) {
    ...
    try {
        if (QA_TEST.ClickListener) {
            document.querySelector('.hello').setAttribute('hello')
        }
        ...
        if (clickedElement.innerText != "In-Store Pickup" && clickedElement.innerText != "Add to wishlist" && clickedElement.innerText != "In-Store Pickup") {
            ...
            if (INC.config.pageType == "pdp") {
                ...
                if (clickedElement.id == "buyProduct" || clickedElement.innerText == "Add to Bag" || (clickedElement.innerText == "ADDED" && clickedElement.parentNode.parentNode.parentNode.parentNode.querySelector('button') != null) || (clickedElement.innerText == "ADDED" && clickedElement.parentNode.parentNode.parentNode.querySelector('button') != null) || (clickedElement.innerText == "Added" && clickedElement.parentNode.parentNode.parentNode.querySelector('button') != null)) {
                    if (INC.config.pageType == "pdp") {
        ...
        if (clickedElement.id == "burger" || clickedElement.parentNode.id == "burger") {
            document.querySelector('body').classList.add('client_menu_visible')
        }
        if (clickedElement.id == "close" || clickedElement.parentNode.id == "close") {
            document.querySelector('body').classList.remove('client_menu_visible')
        }
    } catch (err) {
        ...
        // xhr.send(formData);
        var prdError = INC.methods.getProductIdFromWebPage()
        AddError(prdError, err.stack.toString())
    }
}
        ...
        if (prod_wrapper.querySelector('a') != null) {
            ...
            var img_elem = setInterval(function () {
                ...
                if (circle_img != null) {
                    ...
                    if (p_activePrice.includes('£')) {
                        regularPrice = p_activePrice.split('£')[1]
                        p_activePrice = p_activePrice.split('£')[0]
                    }
                    var subtotalinc_active = document.querySelector('.inc_cart_added_product_desc_subtotal_price_active_text');
...
function AddError(prdid, error) {
    ...
    // Get Page ID
    var epage = INC.methods.detectPageType()
    var currentPageType = ""
    switch (epage) {
        case "pdp":
            currentPageType = "100";
...
// Out Of Stock
function oosproduct(product_id) {
...
function incPriceMismatch(price, special_price) {
    ...
    var price_block = document.querySelector('.price-info')

    var active_price = price_block.querySelector('.Price').innerText.split('£')[1].trim().replace(',', '')
    var old_price = price_block.querySelector('.rrp') !== null ? price_block.querySelector('.rrp').innerText.split('£')[1].trim().replace(',', '') : null



    priceMismatchObj.product_price = old_price !== null ? old_price : active_price
    priceMismatchObj.special_price = old_price !== null ? active_price : null
